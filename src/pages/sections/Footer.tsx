		...
		<footer
			...
			<header className="pointer-events-none mt-15 flex w-full max-w-7xl flex-col">
				...
				<svg
					...
					<path d="M0 15.5 L3000 15.5"></path>
				</svg>
			</header>
			...
			<footer className="pointer-events-none -mt-10 flex w-full max-w-7xl flex-col">
				...
				<div className="flex w-full max-w-7xl flex-col-reverse items-center justify-between gap-5 px-10 sm:flex-row">
					<p className="pointer-events-auto w-[165px] text-center sm:mt-10">
						<span>&copy; {new Date().getFullYear()}&nbsp;</span>
						<a href={`mailto:${Business.mailto}`} className="hover:text-theme-accent-100 cursor-pointer underline duration-300">
							{Business.name}
						</a>
					</p>
					<nav>
						<ul className="pointer-events-auto -mb-5 flex flex-row justify-center gap-10 sm:mt-10 sm:mb-auto">
							{BusinessLinks.map((item, index) => (
								<li key={index} className="group text-center">
									<a href={item.url} className="cursor-pointer" target="_blank" rel="noreferrer">
										<AnimatedIcon width={40} height={40} animationData={item.animationLightData} />
										<span className="text-xs opacity-0 duration-300 group-hover:opacity-100">{item.name}</span>
									</a>
								</li>
							))}
						</ul>
					</nav>
					<nav className="flex w-[165px] justify-center text-center sm:justify-end sm:text-right">
						<ul className="menu">
							<li>
								<NavLink to="/imprint" className="pointer-events-auto mt-10 cursor-pointer">
									Imprint
								</NavLink>
							</li>
						</ul>
